...
// A tiny malicious program that sets return data, then CPIs into Scope's refresh_chainlink_price.
// For simplicity, we implement it inline as a processor here.
mod malicious {
    use super::*;
    solana_program::declare_id!("Maliciou5ReturnData11111111111111111111111111");

    pub fn processor(_program_id: &Pubkey, accounts: &[AccountInfo], ix_data: &[u8]) -> ProgramResult {
        // ix_data layout:
        // [0..4]   u32 len of malicious return data
        // [..]     malicious return data bytes (to be consumed by Scope)
        // [..]     remaining bytes are Anchor-encoded data for Scope::refresh_chainlink_price
        use solana_program::{program::invoke, program_error::ProgramError, program_memory::sol_memcpy};
        ...
        // Remaining bytes contain scope instruction data
        let scope_ix_data = &ix_data[4 + len..];

        // Accounts expected: pass-through for the scope instruction
        // We assume tests construct proper metas for scope accounts.
        let scope_program_id = accounts[0].key; // first account meta is scope program id (readonly)

        // Rebuild the CPI instruction with the same accounts slice (skip first which is this program)
...
mod verifier_stub {
    ...
    pub fn processor(_program_id: &Pubkey, _accounts: &[AccountInfo], _ix_data: &[u8]) -> ProgramResult {
...
async fn test_return_data_confusion_with_injector_cpi() {
    ...
    // Initialize Scope
    let mut ix_data = scope::instruction::Initialize { feed_name: feed_name.clone() }.data();
    let mut init_ix = Instruction {
        program_id: scope_program_id,
    ...
    // Create config PDA via create_account_with_seed? Anchor will init via CPI, so we just need to include it as PDA; program will create it.
    ...
    tx4.sign(&[&payer], recent_blockhash);
    ...
    // We will call malicious program which sets return data and then CPIs to Scope with the above data/accounts
    let mut malicious_ix_data = Vec::with_capacity(4 + malicious_bytes.len() + refresh_data.len());
    ...
    malicious_ix_data.extend_from_slice(&refresh_data);

    // Compose accounts: [scope_program_id (readonly placeholder), then the actual accounts for refresh]
    let mut accounts = vec![AccountMeta::new_readonly(scope_program_id, false)];
    ...
    accounts.extend(refresh_accounts.to_account_metas(None));
    ...
    let mut tx5 = Transaction::new_with_payer(&[malicious_ix], Some(&payer.pubkey()));
    tx5.sign(&[&payer, &user], recent_blockhash);
